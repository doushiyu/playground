...
\0";
	...
	int root_id = 0;
	int global_shifts[MAX_TXT_SIZE - MAX_PATTERN_SIZE] = { -1 };
	...
	/*for (i = 0; i < MAX_TXT_SIZE - 1; i++)
	{
		t[i] = alphabet[(int)rand() % alphabetLength];
	}*/
	fopen_s(&txtfile, "textfile2.txt", "r");
	fread(t, MAX_TXT_SIZE+1, 100000, (FILE*)txtfile);
	...
	fflush(stdout);
	counter = 0;
	shifts = (int*)malloc((size - m + 1) * sizeof(int));
	if (shifts == NULL)
	{
	...
	//if (id == root_id)
		//recv_buffer = shifts;
	MPI_Gather(shifts, 1, MPI_INT, global_shifts + id, 1, MPI_INT, root_id, MPI_COMM_WORLD);
	if (counter == 0)
	{
	...
	if (id == root_id) {
		for (i = 0; i < num_p; i++)
		{
			if (global_shifts[id])
			{
				printf("id %d global count: %d, first shift: %d\n", id, global_counter, global_shifts[i]);
				fflush(stdout);
				break;
			}
